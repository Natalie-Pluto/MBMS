...
import databaseutility.RemovingSession;
...
public class CinemaSessionRemoverTest {

    //class to be tested
    private CinemaSessionRemover cinemaSessionRemover;
    ...
    public void collectCinemaName_cinemaExistsOnFirstAttempt_returnCinemaName() {
        ...
        cinemaSessionRemover = new CinemaSessionRemover(scanner, db);
        try(MockedStatic<CheckIfCinemaExists> cinemaChecker = Mockito.mockStatic(CheckIfCinemaExists.class)){
            ...
            Assert.assertEquals(cinemaSessionRemover.collectCinemaName(), userInput);
        }
    }
    ...
    public void collectCinemaName_cinemaExistsOnSecondAttempt_returnCinemaName() {
        ...
        try(MockedStatic<CheckIfCinemaExists> cinemaChecker = Mockito.mockStatic(CheckIfCinemaExists.class)){
            ...
            Assert.assertEquals(cinemaSessionRemover.collectCinemaName(), secondCinema);
        }
    }
    ...
    public void collectMovieName_movieExistsOnFirstAttempt_returnMovieName() {
        ...
        try(MockedStatic<CheckIfMovieExists> movieChecker = Mockito.mockStatic(CheckIfMovieExists.class)){
            ...
            Assert.assertEquals(cinemaSessionRemover.collectMovieName(), userInput);
        }
    }
    ...
    public void collectMovieName_movieExistsOnSecondAttempt_returnMovieName() {
        ...
        try(MockedStatic<CheckIfMovieExists> movieChecker = Mockito.mockStatic(CheckIfMovieExists.class)){
            ...
            Assert.assertEquals(cinemaSessionRemover.collectMovieName(), movieName);
        }
    }
    ...
    public void collectScreenType_screenTypeValidOnFirstAttempt_returnScreenType() {
        ...
        cinemaSessionRemover = new CinemaSessionRemover(scanner, db);
        assertEquals(cinemaSessionRemover.collectScreenType(), userInput);
    }
    ...
    public void collectScreenType_screenTypeValidOnSecondAttempt_returnScreenType() {
        ...
        cinemaSessionRemover = new CinemaSessionRemover(scanner, db);
        assertEquals(cinemaSessionRemover.collectScreenType(), screenType);
    }
    ...
    public void collectStartTime_startTimeValidOnFirstAttempt_returnStartTime() {
        ...
        cinemaSessionRemover = new CinemaSessionRemover(scanner, db);
        assertEquals(cinemaSessionRemover.collectStartTime(), userInput);
    }

    @Test
    public void run_allInputsValidAndSessionExistsOnFirstAttempt_returnTrue() {

        scanner = new Scanner(System.in);

        //create a spy "spySessionInserter" for CinemaSessionInserter instance
        CinemaSessionRemover cinemaSessionRemover = new CinemaSessionRemover(scanner, db);
        CinemaSessionRemover spySessionRemover = Mockito.spy(cinemaSessionRemover);

        Mockito.doReturn("Town Hall").when(spySessionRemover).collectCinemaName();
        Mockito.doReturn("Ip Man 3").when(spySessionRemover).collectMovieName();
        Mockito.doReturn("Silver").when(spySessionRemover).collectScreenType();
        Mockito.doReturn("2021-11-03 18:16:16").when(spySessionRemover).collectStartTime();
        ...
        try (MockedStatic<CheckIfSessionExists> movieChecker = Mockito.mockStatic(CheckIfSessionExists.class)) {
            ...
            try(MockedStatic<RemovingSession> removeSession = Mockito.mockStatic(RemovingSession.class)){
                removeSession.when(() -> RemovingSession.removeSession(db, "Town Hall", "Ip Man 3", "Silver", "2021-11-03 18:16:16")).thenReturn(true);
                Assert.assertEquals(spySessionRemover.run(), true);
            }
        }
    }
    ...
    public void run_allInputsValidAndSessionNotExistsOnFirstAttempt_returnTrue() {
        ...
        try (MockedStatic<CheckIfSessionExists> movieChecker = Mockito.mockStatic(CheckIfSessionExists.class)) {
            ...
            try(MockedStatic<RemovingSession> removeSession = Mockito.mockStatic(RemovingSession.class)){
                removeSession.when(() -> RemovingSession.removeSession(db, "Town Hall", "Ip Man 3", "Silver", "2021-11-03 18:16:16")).thenReturn(true);
                Assert.assertEquals(spySessionRemover.run(), false);
            }
        }
    }
}